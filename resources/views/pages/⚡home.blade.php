...
<flux:main>
    ...
    <section class="py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl text-center">
                <flux:badge color="blue" class="!rounded-full !px-3 !py-1 !text-xs">核心架构</flux:badge>
                <flux:heading size="lg" class="mt-6">多租户架构，数据物理隔离</flux:heading>
                <flux:text class="mt-4">每个租户拥有独立数据库和专属域名，从底层保障数据安全与隐私合规。</flux:text>
            </div>
    ...
    <section class="border-t border-zinc-100 bg-zinc-50/50 py-24 dark:border-zinc-800/50 dark:bg-zinc-900/50">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl text-center">
                <flux:badge color="purple" class="!rounded-full !px-3 !py-1 !text-xs">强大特性</flux:badge>
                <flux:heading size="lg" class="mt-6">开箱即用的全栈能力</flux:heading>
                <flux:text class="mt-4">Filament 一行命令生成 CRUD，Filament Shield 零代码分配权限——从零到上线只需几天。</flux:text>
            </div>
            <div class="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-amber-200 hover:shadow-lg dark:hover:border-amber-800">
                    ...
                    <flux:heading class="mt-5">Filament 管理面板</flux:heading>
                    <flux:text class="mt-2">强大的管理面板，统一管理用户、团队、租户、角色和权限，支持自定义扩展</flux:text>
                </flux:card>
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-pink-200 hover:shadow-lg dark:hover:border-pink-800">
                    ...
                    <flux:heading class="mt-5">Livewire + Flux UI</flux:heading>
                    <flux:text class="mt-2">零 JavaScript 实现全栈响应式交互，Flux 组件库开箱即用，开发效率数倍提升</flux:text>
                </flux:card>
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-purple-200 hover:shadow-lg dark:hover:border-purple-800">
                    ...
                    <flux:heading class="mt-5">Filament Shield RBAC</flux:heading>
                    <flux:text class="mt-2">自动扫描资源、页面、小组件生成细粒度权限，后台点点鼠标即可完成角色分配</flux:text>
                </flux:card>
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-green-200 hover:shadow-lg dark:hover:border-green-800">
                    ...
                    <flux:heading class="mt-5">团队协作</flux:heading>
                    <flux:text class="mt-2">创建团队、邀请成员、分配角色，团队之间数据隔离，权限管理内置</flux:text>
                </flux:card>
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-rose-200 hover:shadow-lg dark:hover:border-rose-800">
                    ...
                    <flux:heading class="mt-5">认证与安全</flux:heading>
                    <flux:text class="mt-2">双因素认证、Passkeys 无密码登录、CSRF/XSS/SQL 注入防护，全方位安全保障</flux:text>
                </flux:card>
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-orange-200 hover:shadow-lg dark:hover:border-orange-800">
                    ...
                    <flux:heading class="mt-5">国际化</flux:heading>
                    <flux:text class="mt-2">内置中英文双语支持，轻松扩展更多语言</flux:text>
                </flux:card>
            </div>
        </div>
    </section>
    ...
    <section class="py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl text-center">
                <flux:badge color="green" class="!rounded-full !px-3 !py-1 !text-xs">业务场景</flux:badge>
                <flux:heading size="lg" class="mt-6">覆盖多种业务场景</flux:heading>
                <flux:text class="mt-4">多租户 + 独立数据库 + 独立域名的架构，一套代码服务成百上千个独立站点。</flux:text>
            </div>
            <div class="mt-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-lg dark:hover:border-blue-800">
                    ...
                    <flux:heading class="mt-5">站群系统</flux:heading>
                    <flux:text class="mt-2">几百个站点，一个 git pull 全部更新</flux:text>
                </flux:card>
                {{-- 企业官网平台 --}}
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-indigo-200 hover:shadow-lg dark:hover:border-indigo-800">
                    ...
                    <flux:heading class="mt-5">企业官网平台</flux:heading>
                    <flux:text class="mt-2">服务商模式：开发一次，卖给 N 个客户</flux:text>
                </flux:card>
                {{-- 多品牌电商 --}}
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-amber-200 hover:shadow-lg dark:hover:border-amber-800">
                    <div class="flex size-12 items-center justify-center rounded-xl bg-gradient-to-br from-amber-100 to-amber-200 dark:from-amber-900/50 dark:to-amber-800/50">
                    ...
                    <flux:heading class="mt-5">多品牌电商</flux:heading>
                    <flux:text class="mt-2">品牌独立运营，数据安全隔离，集中管控</flux:text>
                </flux:card>
                {{-- 多校区管理 --}}
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-green-200 hover:shadow-lg dark:hover:border-green-800">
                    ...
                    <flux:heading class="mt-5">多校区管理</flux:heading>
                    <flux:text class="mt-2">校区独立，集团统一，互不干扰</flux:text>
                </flux:card>
                {{-- SaaS 产品创业 --}}
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-purple-200 hover:shadow-lg dark:hover:border-purple-800">
                    ...
                    <flux:heading class="mt-5">SaaS 产品创业</flux:heading>
                    <flux:text class="mt-2">从 0 到上线只需几天，聚焦业务而非架构</flux:text>
                </flux:card>
                {{-- 物业/园区管理 --}}
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-rose-200 hover:shadow-lg dark:hover:border-rose-800">
                    ...
                    <flux:heading class="mt-5">物业/园区管理</flux:heading>
                    <flux:text class="mt-2">一套系统管 N 个小区，物业公司的最爱</flux:text>
                </flux:card>
                {{-- 内容矩阵 --}}
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-teal-200 hover:shadow-lg dark:hover:border-teal-800">
                    ...
                    <flux:heading class="mt-5">内容矩阵</flux:heading>
                    <flux:text class="mt-2">一套 CMS 撑起整个内容帝国</flux:text>
                </flux:card>
                {{-- 行业软件定制 --}}
                <flux:card class="group transition-all duration-300 hover:-translate-y-1 hover:border-zinc-200 hover:shadow-lg dark:hover:border-zinc-700">
                    ...
                    <flux:heading class="mt-5">行业软件定制</flux:heading>
                    <flux:text class="mt-2">CRM、ERP 等按客户分别部署，代码统一维护</flux:text>
                </flux:card>
            </div>
        </div>
    </section>
    ...
    <section class="border-t border-zinc-100 bg-zinc-50/50 py-24 dark:border-zinc-800/50 dark:bg-zinc-900/50">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl text-center">
                <flux:badge color="violet" class="!rounded-full !px-3 !py-1 !text-xs">AI 赋能</flux:badge>
                <flux:heading size="lg" class="mt-6">AI + 多租户的化学反应</flux:heading>
                <flux:text class="mt-4">每个租户的数据是私有的、需要隔离的，而 AI 的能力又是通用的——两者结合，催生高价值场景。</flux:text>
            </div>
            <div class="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
    ...
    <section class="border-t border-zinc-100 bg-zinc-50/50 py-24 dark:border-zinc-800/50 dark:bg-zinc-900/50">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl text-center">
                <flux:badge color="blue" class="!rounded-full !px-3 !py-1 !text-xs">快速开始</flux:badge>
                <flux:heading size="lg" class="mt-6">立即体验</flux:heading>
                <flux:text class="mt-4">在线演示环境已就绪，无需部署，即刻体验全功能。</flux:text>
            </div>
            <div class="mt-12 grid gap-8 sm:grid-cols-2">
                ...
                <div class="group rounded-2xl border border-zinc-200 bg-white p-8 transition-all duration-300 hover:-translate-y-1 hover:border-zinc-300 hover:shadow-xl dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700">
                    ...
                    <div class="mt-6 space-y-3 rounded-xl bg-zinc-50/80 p-5 dark:bg-zinc-800/80">
                        <div class="flex flex-col items-start gap-1 text-sm sm:flex-row sm:items-center sm:justify-between">
                            <span class="shrink-0 text-zinc-400 dark:text-zinc-500">仓库地址</span>
                        ...
                        <div class="flex flex-col items-start gap-1 text-sm sm:flex-row sm:items-center sm:justify-between">
                            <span class="shrink-0 text-zinc-400 dark:text-zinc-500">许可证</span>
    ...
    <section class="relative overflow-hidden border-t border-zinc-100 dark:border-zinc-800/50">
        ...
        <div class="relative mx-auto max-w-3xl px-4 py-24 text-center sm:px-6 lg:px-8">
            <flux:heading size="lg">准备好构建你的 SaaS 了吗？</flux:heading>
            <flux:text class="mt-4 !text-lg">克隆项目，一条命令初始化，明天就能给你的客户演示。</flux:text>
            <div class="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
