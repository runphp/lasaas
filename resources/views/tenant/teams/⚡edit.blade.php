...
new class extends Component
{
    ...
    public function updateTeam(): void
    {
        ...
        $this->redirectRoute('tenant.teams.edit', ['team' => $this->teamModel->fresh()->slug], navigate: true);
    }
...
<section class="w-full">
    ...
    <x-tenant::settings.layout :heading="__('Teams')" :subheading="__('Manage your team settings')">
                    ...
                    @foreach ($members as $member)
                        <div class="flex items-center justify-between rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900" data-test="member-row">
                            <div class="flex items-center gap-4">
                                <flux:avatar :src="auth()->user()->avatarUrl" :name="$member['name']" :initials="strtoupper(substr($member['name'], 0, 1))" />
                                <div>
                        ...
                        @if ($member['role'] !== 'owner' && $this->permissions->canRemoveMember)
                            <livewire:tenant::teams.remove-member-modal
                                :team="$teamModel"
                                :member-id="$member['id']"
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                        @foreach ($invitations as $invitation)
                            ...
                            @if ($this->permissions->canCancelInvitation)
                                <livewire:tenant::teams.cancel-invitation-modal
                                    :team="$teamModel"
    ...
    @if ($this->permissions->canCreateInvitation)
        <livewire:tenant::teams.invite-member-modal :team="$teamModel" />
    @endif

    @if ($this->permissions->canDeleteTeam && ! $teamData['is_personal'])
        <livewire:tenant::teams.delete-team-modal :team="$teamModel" />
    @endif
</section>
